...
本项目提供了详细的示例代码，位于 `example/` 目录下，涵盖了从基础解码到可视化的各个方面：

*   **[01_basic_decoding.py](example/01_basic_decoding.py)**: 基础文件解码入门。
*   **[02_stream_decoding.py](example/02_stream_decoding.py)**: 内存/网络流式数据处理。
*   **[03_metadata_config.py](example/03_metadata_config.py)**: 获取 PTS、分辨率、NAL Header 等元数据及参数配置。
*   **[04_image_processing.py](example/04_image_processing.py)**: 将帧保存为图片、访问 YUV 原始数据。
*   **[05_visualization.py](example/05_visualization.py)**: H.265 编码结构可视化（Coding Blocks, Motion Vectors 等）。
